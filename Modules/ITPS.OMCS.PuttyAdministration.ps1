...
function New-PuttySessionRegFile
{
  <#
    .SYNOPSIS
    Creates a registry file that can be merged into Putty.

    .DESCRIPTION
    Imports a csv list of devices and creates a registry file.  That file can be saved for later use or if you want to have create smaller Putty session, you can create multiple registry files for differnt devices.
    An easy way to move your devices or clean up your putty session.  CSV files are smaller and have less risk when emailing than a .reg file.

    .PARAMETER ImportFile
    This is the CSV file to import

    The file format should look this this in notepad:
    "HostName","ipaddress","LogFileName"
    "SwitchName","SwitchIpAddress","C:\\Temp\\putty-&H-&Y&M&D-&T.log"

    .PARAMETER CreateTemplate
    This can be used to create an import file template

    .EXAMPLE
    New-PuttySessionRegFile -ImportFile finename.csv
    Creates a registry file based on the data in the filename.csv 

    .EXAMPLE
    New-PuttySessionRegFile -CreateTemplate
    Creates a csv import file template
  ...
  param
  (
    [Parameter(Mandatory = $true,HelpMessage = "CSV file format.  Column Headings: 'HostName','ipaddress','LogFileName'",ParameterSetName='Set1')]
    [ValidateScript({
          $_ -like '*.csv'
    })]
    [String]$ImportFile,
    [Parameter(Mandatory = $true,HelpMessage = 'To create a template import file',ParameterSetName='Set2')]
    [Switch]$CreateTemplate
  )
  
  [String]$OutputFile = ('{0}\Desktop\PuttySessions({1}).reg' -f $env:userprofile, $(Get-Date -UFormat %j%S))  
  
  $ImportedSwitches = Import-Csv -Path $ImportFile -UseCulture -ErrorAction Stop
  
  
  if($CreateTemplate)
  {
    [String]$ExampleCsv = ('{0}\Desktop\PuttySessionsImportExample.csv' -f $env:userprofile)
    
    $null = (New-Item -Path $ExampleCsv  -ItemType File -Force -Verbose)
    
    $Obj = [pscustomobject]@{
      'HostName'  = 'SwitchName'
      'ipaddress' = 'SwitchIpAddress'
      'LogFileName' = 'C:\\Temp\\putty-&H-&Y&M&D-&T.log'
    }

    $Obj | Export-Csv -Path $ExampleCsv -NoTypeInformation -Verbose
    
    Write-Verbose -Message ('Find Example CSV file here: {0}' -f $ExampleCsv) -Verbose
    Return
  }


    "Windows Registry Editor Version 5.00`n" | set-Content -Path $OutputFile
    
    $ImportedSwitches = Import-Csv -Path $ImportFile -UseCulture
    
    foreach($switchData in $ImportedSwitches)
    {
      $SwitchName = $switchData.'HostName'
      $SwitchIp = $switchData.'ipaddress'
      $PuttyLog = $switchData.'LogFileName'
      $PuttyBlurb = (@'
...
'@ -f $SwitchName, $SwitchIp, $PuttyLog, '255,128,128')
      $PuttyBlurb | Add-Content -Path $OutputFile
    } 
  

  Write-Host -Object ('Find registry file here: {0}' -f $(Get-Item -Path $OutputFile).FullName)
}
